...
function unsealBytes(env: SealedEnvelope, encPrivateRaw: Buffer): Buffer {
  if (env.scheme !== SEALED_SCHEME) throw new Error(`unknown sealing scheme: ${env.scheme}`);
  const priv = x25519PrivateFromRaw(encPrivateRaw);
  const epk = x25519PublicFromRaw(Buffer.from(env.epk, 'base64'));
  const key = deriveKey(diffieHellman({ privateKey: priv, publicKey: epk }));
  const ctFull = Buffer.from(env.ct, 'base64');
  const tag = ctFull.subarray(ctFull.length - 16);
  const enc = ctFull.subarray(0, ctFull.length - 16);
  const decipher = createDecipheriv('chacha20-poly1305', key, Buffer.from(env.nonce, 'base64'), { authTagLength: 16 });
...
export function sealPayloads(
  bundle: Record<string, JsonValue>, recipientEncPubKey: string,
  ...
  for (const ev of (out.events as Array<Record<string, JsonValue>>) ?? []) {
    ...
    if (!predicate || predicate(ev)) {
      ev.payload = { chp_sealed: sealBytes(recipientEncPubKey, canonBytes(ev.payload)) as unknown as JsonValue };
    }
  }
  return out;
}

/** Decrypt one `{chp_sealed}` marker to its plaintext payload. */
export function unsealPayload(marker: Record<string, JsonValue>, encPrivateRaw: Buffer): JsonValue {
  const env = marker?.chp_sealed as unknown as SealedEnvelope | undefined;
