...
Status: **released** (v0.2 2026-07-06; v0.2.1–v0.2.9 additions 2026-07-09/11). Changes via [proposals/](proposals/) — see [CHANGELOG.md](CHANGELOG.md). **Normative.** Additive over [v0.1](chp-v0.1.md); refines the
...
result (`"replayed": true`) and runs NOTHING below — including denials: a
replayed denial is the same denial, and gates do not re-run because their
decision is part of the recorded outcome. No events are emitted for an
execution that did not happen. Transport auth (binding §2) has already run —
replay never bypasses it. Streaming mode is excluded.
...
**Absolute deadline (proposal 0052, optional).** A host that supports absolute
deadlines evaluates the envelope's optional `deadline` in the budget tier (with
gate 8, before execution): if the deadline has passed — skew-aware (TIME-003), and
**fail-closed** under clock uncertainty (TIME-006) — the invocation is `denied`
`deadline_exceeded` (`retryable: false`) and no effect runs. The deadline is an
**absolute** instant, propagated to sub-invocations **unchanged** so an end-to-end
budget is never re-based per hop. An absent `deadline` changes nothing (byte-identical
pre-0052 behavior).
