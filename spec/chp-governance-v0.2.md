...
| `mandate_exhausted` | A mandate's signed `max_invocations` cap was reached — the delegate has already used the grant its limit of distinct invocations (chp-v0.2.md §10, proposal 0026). NOT retryable: the grant is spent; a new mandate is a new object. `details` SHOULD carry `used` and `max_invocations`. | false |
| `capability_version_unsupported` | The capability exists but no registered version satisfies the caller's requested version range (chp-v0.2.md §6, proposal 0028). NOT retryable against this host: the host offers what it offers. `details` SHOULD carry `requested` (the range) and `available` (the registered versions). | false |
